...
__global__ void ASM_kernel(char *g_input_string, int input_size, int *g_pattern_decode, int pattern_length, int no_of_patterns, int b, int *g_matched_result)
{
    ...
    //int gbid = (gridDim.x * blockIdx.x) + blockIdx.y;
    //int start = gbid*THREAD_BLOCK_SIZE + tid;
    int start = gbid;
    
    //__shared__ unsigned int bit_vector = 0;
    __shared__ unsigned int bit_vector_result[256];
    __shared__ unsigned int bit_vector;

  int pow_2b = 1 << b; 
  //unsigned int bit_vector = 0;
        if (g_input_string[ start + tid ] == 'A') {
            bit_vector_result[tid] = g_pattern_decode[4*tid];
        }
        else if (g_input_string[ start + tid ] == 'C'){
            bit_vector_result[tid] = g_pattern_decode[4*tid+1];
        }
        else if (g_input_string[ start + tid ] == 'G'){
            bit_vector_result[tid] = g_pattern_decode[4*tid+2];
        }
        else if (g_input_string[ start + tid ] == 'T'){   //case of G
            bit_vector_result[tid] = g_pattern_decode[4*tid+3];
        }
	else {
	     bit_vector_result[tid] = 0;
	}
	__syncthreads();

    //cuPrintf("threadIdx.x = %d, blockIdx.y = %d \t gridDim.x = %d, blockIdx.x = %d, grid = %d, bit_vector = %d, character = %s \n", threadIdx.x, blockIdx.y, gridDim.x,  blockIdx.x,gbid, bit_vector_result[tid], g_input_string[start + tid]);
    //g_matched_result[start] = bit_vector;

	
  if(tid == 0) {
    for (int i = 0; i<pattern_length; i++) {
	bit_vector = bit_vector + bit_vector_result[i];
    }
    for (int j = no_of_patterns-1; j >= 0; j--) {
        g_matched_result[start*no_of_patterns + j] = bit_vector % pow_2b;
...
void ASM_process_top (char *g_input_string, size_t input_size,  int *g_pattern_decode, int pattern_length, int no_of_patterns, int b, int *g_matched_result)
{
    ...
    //int num_blocks = (input_size-pattern_length)/THREAD_BLOCK_SIZE + 1 ;
    //int num_blocks = (input_size-pattern_length)/(pattern_length+1) + 1 ;
    int num_blocks = input_size-pattern_length + 1;


        //dim3  dimBlock( THREAD_BLOCK_SIZE, 1 ) ;
        dim3  dimBlock( pattern_length , 1 ) ;
        dim3  dimGrid ;
